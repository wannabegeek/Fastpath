...
namespace fp {
    ...
    class shm_peer_connection : public peer_connection {
        ...
        const std::function<void(const std::function<void(const subject<> &, const message_wrapper &)>)> &m_message_reader;
...
//        std::function<void(PeerConnection *connection)> m_msg_callback;
    ...
    public:
        ...
        shm_peer_connection(Queue *queue, InterprocessNotifierServer::notifier_type &&notifier, std::unique_ptr<UnixSocket> &&socket, int process_id, SharedMemoryManager *manager, const std::function<void(const std::function<void(const subject<> &, const message_wrapper &)>)> &message_reader);
